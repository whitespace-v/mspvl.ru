...
const Admin = () => {
	...
	const createCatalogCategoryHandler = () => {
		if (catalogCategoryImage && catalogCategoryName) {
			dispatch(createCatalogCategory(catalogCategoryName, catalogCategoryImage))
				.then(() => dispatch(fetchCatalogCategories())
				);
	...
	const createCatalogProductHandler = () => {
		if (
			...
			dispatch(createCatalogProduct(currentCatalogCategory, catalogProductName))
				.then(() => dispatch(fetchCatalogCategories())
				);
		...
		<Background>
			...
			<Layout>
				<div className={classes['Admin']}>
					...
					<div className={classes['Admin__item']}>
						<div className={classes['Admin__item-title']}>Каталог категорий прайс-листа</div>
						<hr />
						<div className={classes['Admin__item-categories']}>
							{catalogCategories.map((i) => (
								<div className={i.id === currentCatalogCategory.id
									? classes['Admin__item-categories-item-active']
									: classes['Admin__item-categories-item']
								}
									 key={i.id}
								...
								>
									...
									<UIButton type={'text'}
											  onClick={() => dispatch(deleteCatalogCategory(i))
												  .then(() => dispatch(fetchCatalogCategories()))
											  }
									>
										Удалить
						...
						{creationCatalogCategory ? (
							<div className={classes['Admin__item-categories-form']}
								 style={creationCatalogCategory && { flexDirection: 'column' }}
							>
								<div className={'UIInput'}>
									<input required value={catalogCategoryName}
										   onChange={(e) => setCatalogCategoryName(e.currentTarget.value)}
									/>
									<div className={'UIInput-placeholder'}>Категория</div>
								</div>
								<label className={classes['file']}>
									<input placeholder={'Добавить файл'} type='file'
					...
					<div className={classes['Admin__item']}>
						<div className={classes['Admin__item-title']}>Каталог товаров прайс-листа</div>
						<hr />
						<div className={classes['Admin__item-categories']}>
							{catalogProducts.map((i: any) => (
								<div key={i.id}
									 className={i === currentCatalogCategory
										 ? classes['Admin__item-categories-item-active']
										 : classes['Admin__item-categories-item']
									 }
								>
									...
									<UIButton type={'text'}
											  onClick={() =>
												  dispatch(deleteCatalogProduct(i))
													  .then(() => dispatch(fetchCatalogCategories()))
											  }
									>
										Удалить
